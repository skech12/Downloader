...
# Top-level browse categories — same order as website (index.html CANONICAL_SITE_CATEGORIES)
# and dataUpload/UploadAndUpdate.py CATEGORIES keys.
SITE_TAXONOMY_ORDER = (
    "Visual / Image",
...
def load_data():
    ...
    Category order matches the website browse sidebar: canonical taxonomy first,
    then any bucket-only folder names not in the taxonomy (by file count desc).
    """
    with console.status("[bold cyan]Loading data…"):
        data = api_get("/api/tui/files")
    files_data = data.get("files") or {}
    file_counts = dict(data.get("file_counts") or {})
    total_datasets = data["total"]
    taxonomy_set = set(SITE_TAXONOMY_ORDER)
    extras = sorted(
        [k for k in files_data if k not in taxonomy_set],
        key=lambda k: (-file_counts.get(k, 0), k.lower()),
    )
    categories = list(SITE_TAXONOMY_ORDER) + extras
    for c in SITE_TAXONOMY_ORDER:
        file_counts.setdefault(c, 0)
